...
struct BalanceMerkleCache {
    ...
    valid: bool,
    /// The last computed root hash (with mix_in_length), used to validate
    /// that the cache corresponds to the expected parent state on the caller's fork.
    last_root: [u8; 32],
}
...
impl BalanceMerkleCache {
    ...
    fn finalize_root(&mut self, balance_count: usize) -> [u8; 32] {
...
/// `expected_prev_hash` is the hash the caller expects the cache to currently hold.
/// This validates that the cache corresponds to the correct fork/parent state.
///
/// The cache must have been initialized by a prior `hash_balances_incremental` call.
/// If the cache is invalid, balance_count doesn't match, or the expected hash doesn't
/// match the cache's last computed root, returns None (caller falls back to full hash).
pub fn apply_updates_and_hash(
    updates: &[(u32, u64)],
    balance_count: usize,
    ...
    let mut cache = BALANCE_CACHE.lock().unwrap();
    if !cache.valid
        || cache.balances.len() != balance_count
        || &cache.last_root != expected_prev_hash
    {
        return None;
    }
...
mod tests {
    ...
    fn targeted_updates() {
        ...
        let h2 = apply_updates_and_hash(&updates, 1000, &_h1).unwrap();
        assert_eq!(h2, reference_hash(&balances));
    }
    ...
    fn targeted_updates_returns_none_when_invalid() {
        ...
        assert_eq!(apply_updates_and_hash(&updates, 100, &fake_hash), None);
    }

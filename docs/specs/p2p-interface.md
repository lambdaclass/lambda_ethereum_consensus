...
This document contains the networking specification for the Ethereum consensus layer, covering all forks from Phase 0 through Fulu (PeerDAS). It incorporates changes introduced by Altair, Bellatrix, Capella, Deneb, Electra, and Fulu into a single consolidated reference.
...
- [Consensus-layer network interaction domains](#consensus-layer-network-interaction-domains)
  ...
  - [The discovery domain: discv5](#the-discovery-domain-discv5)
    ...
    - [ENR structure](#enr-structure)
      ...
      - [`syncnets` bitfield](#syncnets-bitfield)
...
|------------------------------------------|-----------------------------------|---------------------------------------------------------------------|
| `KZG_COMMITMENT_INCLUSION_PROOF_DEPTH`   | `uint64(floorlog2(get_generalized_index(BeaconBlockBody, 'blob_kzg_commitments')) + 1 + ceillog2(MAX_BLOB_COMMITMENTS_PER_BLOCK))` (= 17) | <!-- predefined --> Merkle proof depth for `blob_kzg_commitments` list item (Deneb) |
| `KZG_COMMITMENTS_INCLUSION_PROOF_DEPTH` | `uint64(floorlog2(get_generalized_index(BeaconBlockBody, 'blob_kzg_commitments')))` (= 4) | *[New in Fulu:EIP7594]* Merkle proof index for `blob_kzg_commitments` (used in data column inclusion proofs) |
...
| `MAX_REQUEST_BLOCKS_DENEB`               | `2**7` (= 128)                    | Maximum number of blocks in a single request (Deneb and later) |
| `MAX_REQUEST_BLOB_SIDECARS_ELECTRA`      | `MAX_REQUEST_BLOCKS_DENEB * MAX_BLOBS_PER_BLOCK_ELECTRA` | *[New in Electra:EIP7691]* Maximum number of blob sidecars in a single request |
...
| `BLOB_SIDECAR_SUBNET_COUNT`              | `6`                               | The number of blob sidecar subnets used in the gossipsub protocol (pre-Electra). |
| `BLOB_SIDECAR_SUBNET_COUNT_ELECTRA`      | `9`                               | *[New in Electra:EIP7691]* The number of blob sidecar subnets used in the gossipsub protocol (Electra, deprecated in Fulu). |
| `DATA_COLUMN_SIDECAR_SUBNET_COUNT`       | `128`                             | *[New in Fulu:EIP7594]* The number of data column sidecar subnets used in the gossipsub protocol. |
| `MAX_REQUEST_DATA_COLUMN_SIDECARS`       | `MAX_REQUEST_BLOCKS_DENEB * NUMBER_OF_COLUMNS` | *[New in Fulu:EIP7594]* Maximum number of data column sidecars in a single request. |
| `MIN_EPOCHS_FOR_DATA_COLUMN_SIDECARS_REQUESTS` | `2**12` (= 4096 epochs, ~18 days) | *[New in Fulu:EIP7594]* The minimum epoch range over which a node must serve data column sidecars. |
...
*[Modified in Fulu:EIP7594 to add `custody_group_count`]*
...
(
  ...
  custody_group_count: uint64  # [New in Fulu:EIP7594]
)
```
...
- `custody_group_count` represents the node's custody group count. Clients MAY reject peers with a value less than `CUSTODY_REQUIREMENT`. *[New in Fulu:EIP7594]*
...
*[Modified in Fulu to include all forks through Fulu]*
...
| Name                                    | Message Type                     | Fork           |
|-----------------------------------------|----------------------------------|----------------|
| `beacon_block`                          | `SignedBeaconBlock`              | Phase 0+       |
| `beacon_aggregate_and_proof`            | `SignedAggregateAndProof`        | Phase 0+       |
| `beacon_attestation_{subnet_id}`        | `Attestation` / `SingleAttestation` (Electra+) | Phase 0+ |
| `voluntary_exit`                        | `SignedVoluntaryExit`            | Phase 0+       |
| `proposer_slashing`                     | `ProposerSlashing`               | Phase 0+       |
| `attester_slashing`                     | `AttesterSlashing`               | Phase 0+       |
| `sync_committee_contribution_and_proof` | `SignedContributionAndProof`     | Altair+        |
| `sync_committee_{subnet_id}`            | `SyncCommitteeMessage`           | Altair+        |
| `bls_to_execution_change`               | `SignedBLSToExecutionChange`     | Capella+       |
| `blob_sidecar_{subnet_id}`              | `BlobSidecar`                    | Deneb+ (deprecated in Fulu) |
| `data_column_sidecar_{subnet_id}`       | `DataColumnSidecar`              | Fulu+          |
...
- _[REJECT]_ The length of KZG commitments is less than or equal to the limitation defined in Consensus Layer:
  - Prior to Electra: `len(body.signed_beacon_block.message.blob_kzg_commitments) <= MAX_BLOBS_PER_BLOCK`
  - In Electra: `len(signed_beacon_block.message.body.blob_kzg_commitments) <= MAX_BLOBS_PER_BLOCK_ELECTRA`
  - In Fulu and later: `len(signed_beacon_block.message.body.blob_kzg_commitments) <= get_blob_parameters(get_current_epoch(state)).max_blobs_per_block`
...
*[Modified in Electra]* The following convenience variables are re-defined:

- `index = get_committee_indices(aggregate.committee_bits)[0]`

The following validations are added in Electra:

- _[REJECT]_ `len(committee_indices) == 1`, where
...
*[Deprecated in Fulu: `blob_sidecar_{subnet_id}` is deprecated as of `FULU_FORK_EPOCH`. The `data_column_sidecar_{subnet_id}` topic replaces it for Fulu and later.]*
...
- _[REJECT]_ The sidecar's index is consistent with the blob limit:
  - Prior to Electra: `blob_sidecar.index < MAX_BLOBS_PER_BLOCK`.
  - In Electra and later: `blob_sidecar.index < MAX_BLOBS_PER_BLOCK_ELECTRA`.
- _[REJECT]_ The sidecar is for the correct subnet -- i.e. `compute_subnet_for_blob_sidecar(blob_sidecar.index) == subnet_id`.
  The number of subnets is `BLOB_SIDECAR_SUBNET_COUNT` prior to Electra and `BLOB_SIDECAR_SUBNET_COUNT_ELECTRA` from Electra.
...
`kzg_commitments` of the given beacon block. Clients can choose to cache the
...
*[Modified in Electra]* The topic is updated to propagate `SingleAttestation` objects.
...
The following validations are removed in Electra:
...
- `earliest_available_slot`: The slot of the earliest available block (`SignedBeaconBlock`).
...
**Deprecation behavior (from Fulu):**

During the deprecation transition period (before `FULU_FORK_EPOCH + MIN_EPOCHS_FOR_BLOB_SIDECARS_REQUESTS`):

- Clients MUST respond with blob sidecars corresponding to block roots from the range
...
The response MUST contain no more than `count * MAX_BLOBS_PER_BLOCK_ELECTRA` blob sidecars.
...
Clients MUST respond with data column sidecars from their view of the current
...
finalization MUST lead to the finalized block reported in the `Status` handshake.
...
Requests the MetaData of a peer, using the new `MetaData` definition given above
that is extended from Altair with the `custody_group_count` field. Other
conditions for the `GetMetaData` protocol are unchanged from the Altair p2p
networking document.
...
*[Modified in Fulu:EIP7892 to update field value computations for BPO forks]*
...
* `next_fork_version` is the fork version corresponding to the next planned hard fork at a future epoch. The fork version will only change for regular forks, _not BPO forks_. Note that it is possible for the blob schedule to define a change at the same epoch as a regular fork; this situation would be considered a regular fork. If no future fork is planned, set `next_fork_version = current_fork_version` to signal this fact.
* `next_fork_epoch` is the epoch at which the next fork (whether a regular fork _or a BPO fork_) is planned and the `current_fork_version` will be updated. If no future fork is planned, set `next_fork_epoch = FAR_FUTURE_EPOCH` to signal this fact.

defmodule LambdaEthereumConsensus.Network do
  ...
  def init(_) do
    ...
    port = 1024 + rem(System.system_time(), 65535 - 1024)
    {:ok, listener} = Libp2p.listen_v5("0.0.0.0:#{port}", @bootnodes)
    {:ok, iterator} = Libp2p.listener_random_nodes(listener)

    discoverer = Task.start(__MODULE__, :discover_peers, [self(), peerstore, iterator])

    {:ok, {host, discoverer}}
  end

  def discover_peers(consumer, peerstore, iterator) do
    ...
    GenServer.cast(consumer, {:peer_discovered, id})

    discover_peers(consumer, peerstore, iterator)
  end

  @impl true
  def handle_cast({:peer_discovered, id}, {host, discoverer}) do
    Task.start(Libp2p, :host_connect, [host, id])
    {:noreply, {host, discoverer}}
  end
end

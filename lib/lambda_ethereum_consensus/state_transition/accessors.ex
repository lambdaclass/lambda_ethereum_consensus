defmodule LambdaEthereumConsensus.StateTransition.Accessors do
  ...
  def get_committee_count_per_slot(state, epoch) do
    ...
    committee_size = trunc(
      active_validators_count / ChainSpec.get("SLOTS_PER_EPOCH") / ChainSpec.get("TARGET_COMMITTEE_SIZE")
    )
  ...
  """
  @spec get_beacon_committee(BeaconState.t(), SszTypes.slot(), SszTypes.committee_index()) :: list(SszTypes.validator_index())
  ...
  def get_base_reward(state, index) do
    increments = div(state[:validators][index][:effective_balance], ChainSpec.get("EFFECTIVE_BALANCE_INCREMENT"))
  ...
  """
  @spec get_attestation_participation_flag_indices(BeaconState.t(), SszTypes.AttestationData.t(), SszTypes.uint64()) 
  ...
  def get_attestation_participation_flag_indices(state, data, inclusion_delay) do
    ...
    is_matching_target = is_matching_source && data.target.root == get_block_root(state, data.target.epoch)
    is_matching_head = is_matching_target && data.beacon_block_root == get_block_root_at_slot(state, data.slot)
    if not is_matching_source do 
      {:error, "Attestation source does not match justified checkpoint"}
    end

    source_indices = 
      if is_matching_source && inclusion_delay <= round(:math.sqrt(ChainSpec.get("SLOTS_PER_EPOCH"))) do
  ...
  def get_block_root(state, epoch) do
    {_, block_root} = get_block_root_at_slot(state, Misc.compute_start_slot_at_epoch(epoch))
    block_root
  end
  ...
  def get_seed(state, epoch, domain_type) do
    mix = get_randao_mix(state, epoch + ChainSpec.get("EPOCHS_PER_HISTORICAL_VECTOR") - ChainSpec.get("MIN_SEED_LOOKAHEAD") - 1)
    :crypto.hash(:sha256, domain_type <> Helper.uint_to_bytes(epoch) <> mix)
  end
  ...
  def get_indexed_attestation(state, attestation) do
    ...
    # IndexedAttestation.t(sorted_attesting_indices, attestation.data, attestation.signature)
  end

  @doc """
  Return the set of attesting indices corresponding to ``data`` and ``bits``.
  """
  @spec get_attesting_indices(BeaconState.t(), SszTypes.AttestationData.t(), SszTypes.bitlist) :: MapSet.t()
  def get_attesting_indices(state, data, bits) do
    committee = get_beacon_committee(state, data.slot, data.index)
    
    committee
    |> Enum.with_index()
    |> Enum.filter(fn {_index, i} -> Enum.at(bits, i) end)
  ...
  def get_beacon_proposer_index(state) do
    ...
    seed = :crypto.hash(:sha256, get_seed(state, epoch, Constants.domain_beacon_proposer()) + Helper.uint_to_bytes(state.slot))
  ...
  def get_total_balance(state, indices) do
    total_balance = 
      indices
      |> Enum.map(fn index -> Map.get(state.validators[index], :effective_balance) end)
      |> Enum.sum()

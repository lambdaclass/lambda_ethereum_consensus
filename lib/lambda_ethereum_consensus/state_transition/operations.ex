defmodule LambdaEthereumConsensus.StateTransition.Operations do
  ...
  def process_attestation(state, attestation) do
    ...
    with :ok <- (if data.target.epoch not in [Accessors.get_previous_epoch(state), Accessors.get_current_epoch(state)], do: {:error, "Incorrect epoch"}, else: :ok),
      :ok <- (if data.target.epoch != Misc.compute_epoch_at_slot(data.slot), do: {:error, "Epoch mismatch"}, else: :ok),
      :ok <- (if data.slot + ChainSpec.get("MIN_ATTESTATION_INCLUSION_DELAY") > state.slot, do: {:error, "Inclusion delay not met"}, else: :ok),
      :ok <- (if data.index >= Accessors.get_committee_count_per_slot(state, data.target.epoch), do: {:error, "Index exceeds committee count"}, else: :ok),
      :ok <- (if byte_size(attestation.aggregation_bits) != length(Accessors.get_beacon_committee(state, data.slot, data.index)), do: {:error, "Mismatched aggregation bits length"}, else: :ok) do

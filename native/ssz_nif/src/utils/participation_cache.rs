...
impl ParticipationMerkleCache {
    ...
    fn finalize_root(&self, value_count: usize) -> [u8; 32] {
...
/// If the cache is invalid or value_count doesn't match, returns None (caller
...
pub fn apply_participation_updates(
    ...
    let mut cache = get_cache(field_num).lock().unwrap();
    if !cache.valid || cache.values.len() != value_count {
        return None;
    }
...
mod tests {
    ...
    fn targeted_updates() {
        ...
        let h2 = apply_participation_updates(15, &updates, 1000).unwrap();
        assert_eq!(h2, reference_hash(&values));
    }
    ...
    fn targeted_updates_returns_none_when_invalid() {
        ...
        assert_eq!(apply_participation_updates(16, &updates, 100), None);
    }

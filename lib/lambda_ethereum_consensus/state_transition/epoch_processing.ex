defmodule LambdaEthereumConsensus.StateTransition.EpochProcessing do
  ...
  @spec process_bls_to_execution_change(BeaconState.t(), SszTypes.SignedBLSToExecutionChange.t()) ::
          BeaconState.t()
  ...
  def process_bls_to_execution_change(state, signed_address_change) do
    ...
    if address_change.validator_index >= length(state.validators) do
      ...
      if Enum.slice(validator.withdrawal_credentials, 0..0) == ChainSpec.get("BLS_WITHDRAW_PREFIX") do
        domain = Misc.compute_domain(ChainSpec.get("DOMAIN_BLS_TO_EXECUTION_CHANGE"), genesis_validators_root: state.genesis_validators_root)
  ...
  def process_bls_to_execution_change(state, signed_address_change) do
    ...
    if address_change.validator_index < length(state.validators) do
      ...
      if Enum.take(validator.withdrawal_credentials, 0) ==
           ChainSpec.get("BLS_WITHDRAWAL_PREFIX") and
           Enum.slice(validator.withdrawal_credentials, 1..-1) ==
             Enum.slice(:crypto.hash(:sha256, Enum.slice(address_change.from_bls_pubkey, 1..-1)), 1..-1) do
        # Fork-agnostic domain since address changes are valid across forks
        domain =
          Misc.compute_domain(ChainSpec.get("DOMAIN_BLS_TO_EXECUTION_CHANGE"),
        ...
        case Bls.verify(
          ...
          new_withdrawal_credentials =
            <<ChainSpec.get("ETH1_ADDRESS_WITHDRAWAL_PREFIX")::size(8)>> <>
              <<0::size(8)>> <> address_change.to_execution_address
          ...
          {:ok,
           ...
           {:error, error_msg} -> {:error, "Bls verification failed"}
        end
      else
        {:error, "Withdrawal credential validation failed"}
      end
    else

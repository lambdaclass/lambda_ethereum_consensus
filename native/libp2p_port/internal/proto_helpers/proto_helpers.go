...
func ResponseNotification(from []byte, result []byte, err error, protocolId string) *proto_defs.Notification {
	var message []byte
	var success bool
	...
	response := &proto_defs.Response{From: from, Message: message, Success: success, ProtocolId: []byte(protocolId)}

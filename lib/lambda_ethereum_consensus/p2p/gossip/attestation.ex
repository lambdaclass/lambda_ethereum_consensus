defmodule LambdaEthereumConsensus.P2P.Gossip.Attestation do
  ...
  def init(_init_arg) do
    ...
    {:ok, %{attnets: MapSet.new(), fork_context: fork_context, attestations: %{}}}
  end

  @impl true
  def handle_call({:collect, subnet_id}, _from, state) do
    :ok = state.fork_context |> get_topic_name(subnet_id) |> Libp2pPort.subscribe_to_topic()
  ...
  def handle_call({:stop_collecting, subnet_id}, _from, state) do
    if MapSet.member?(state.attnets, subnet_id) do
      :ok = state.fork_context |> get_topic_name(subnet_id) |> Libp2pPort.unsubscribe_from_topic()
  ...
  defp get_topic_name(fork_context, subnet_id) do
